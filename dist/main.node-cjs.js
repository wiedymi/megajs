...
var StreamSkip = _interopDefault(require('stream-skip'));
...
function megaEncrypt(key, options = {}) {
  ...
  let stream$$1 = through(write, end);
  ...
  function end() {
    ...
    stream$$1.key = mergeKeyMac(key, mac);
    this.emit('end');
  }
...
const ERRORS = {
  ...
  18: 'ETEMPUNAVAIL (-18): Resource temporarily not available, please try again later' // The original MEGA package used https://g.api.mega.co.nz/
...
class File extends events.EventEmitter {
  ...
  loadAttributes(cb) {
    ...
    const req = this.directory ? {
      ...
      a: 'g',
      p: this.downloadId // todo: nodeId version ('n')
  ...
  download(options, cb) {
    ...
    const stream$$1 = apiStart === start ? decryptStream : decryptStream.pipe(new StreamSkip({
      skip: start - apiStart
    }));
    ...
    cs.request(req, (err, response) => {
      ...
      function handleMegaErrors(resp) {
        ...
        stream$$1.emit('error', Error('MEGA returned a ' + resp.statusCode + ' status code'));
      }
      ...
      if (maxConnections === 1) {
        ...
        combined.on('error', err => stream$$1.emit('error', err));
  ...
  link(options, cb) {
    ...
    let url$$1 = `https://mega.nz/#${this.directory ? 'F' : ''}!${this.downloadId}`;
...
class MutableFile extends File {
  constructor(opt, storage) {
    ...
    if (opt.k) {
      ...
      try {
        for (var _iterator = idKeyPairs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  upload(opt, source, cb) {
    ...
    const stream$$1 = this._upload(opt, source, 0, checkCallbacks);
  ...
  _upload(opt, source, type, cb) {
    ...
    let stream$$1 = pipeline(pause, encrypter); // Size is needed before upload. Kills the streaming otherwise.
    ...
    if (size) {
      this._uploadWithSize(stream$$1, size, encrypter, pause, type, opt, cb);
    } else {
      stream$$1 = pipeline(detectSize(size => {
        this._uploadWithSize(stream$$1, size, encrypter, pause, type, opt, cb);
      }), stream$$1);
    } // handle stream
  ...
  _uploadAttribute(opt, source, type, cb) {
    const gotBuffer = (err, buffer) => {
      ...
      const pause = through().pause();
      let stream$$1 = pipeline(pause);
      stream$$1.end(buffer);

      this._uploadWithSize(stream$$1, buffer.length, stream$$1, pause, type, opt, cb);
    }; // handle buffer
  ...
  _uploadWithSize(stream$$1, size, source, pause, type, opt, cb) {
    ...
    const sendChunk = () => {
      ...
      httpreq.on('error', error => {
        stream$$1.emit('error', Error('Connection error: ' + error.message));
      });
      httpreq.on('response', response => {
        if (response.statusCode === 200) return;
        stream$$1.emit('error', Error('MEGA returned a ' + response.statusCode + ' status code'));
      });
      uploadBuffer = null;
      position += chunkSize;
    ...
    source.on('end', () => {
      if (size && sizeCheck !== size) {
        stream$$1.emit('error', Error('Specified data size does not match: ' + size + ' !== ' + sizeCheck));
      }
    });
  ...
  link(options, cb) {
    ...
    this.api.request({
      ...
      if (err) return cb(err);
      let url$$1 = `https://mega.nz/#${folderKey ? 'F' : ''}!${id}`;
      if (!options.noKey && this.key) url$$1 += `!${e64(folderKey || this.key)}`;
      cb(null, url$$1);
    });
    return this;
  }
